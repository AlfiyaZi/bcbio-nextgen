...
from bcbio.distributed.transaction import _get_dir_abspath
...
def mock_io(mocker):
    ...
    mocker.patch('bcbio.distributed.transaction.utils.get_size')
...
def test_tx_tmpdir_make_tmp_dir(mock_exists, mock_io):
    with tx_tmpdir():
        pass
    expected_basedir = "%s/bcbiotx" % TMP
...
@mock.patch('bcbio.distributed.transaction._get_dir_abspath')
def test_tx_tmpdir_yields_created_dir(
        mock_get_dir_abspath,
        mock_get_base_tmpdir,
        mock_io):
    data, base_dir = mock.Mock(), mock.Mock()
    with tx_tmpdir(data, base_dir):
        pass
    mock_get_base_tmpdir.assert_called_once_with(data)
    mock_get_dir_abspath.assert_called_once_with(
        mock_get_base_tmpdir.return_value, CWD)

    transaction.utils.safe_makedir.assert_called_once_with(
        mock_get_dir_abspath.return_value)
    transaction.tempfile.mkdtemp.assert_called_once_with(
        dir=mock_get_dir_abspath.return_value)
...
def test_get_dir_abspath__flow(mock_path, mock_io):
    TMP = 'TEST_CONFIG_TMP'
    ...
    result = _get_dir_abspath(TMP, CWD)
...
def test_tx_tmpdir_rmtree_not_called_if_remove_is_false(mock_io):
    with tx_tmpdir(remove=False):
        pass
    assert not transaction.shutil.rmtree.called


def test_tx_tmpdir_rmtree_called_if_remove_is_True(mock_io):
...
def test_file_transaction(mock_io):
    ...
    transaction.tempfile.mkdtemp.return_value = 'TEST_TMPDIR'
    with file_transaction(CONFIG, '/some/path') as tmp_path:
        assert tmp_path == 'TEST_TMPDIR/path'
    transaction.shutil.rmtree.assert_called_once_with(
        'TEST_TMPDIR', ignore_errors=True)
